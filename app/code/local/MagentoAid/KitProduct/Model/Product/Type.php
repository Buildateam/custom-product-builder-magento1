 ...
 */
/**
 * KitProduct Type Model
 *
 * @category    Mage
 * @package     MagentoAid_KitProduct
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class MagentoAid_KitProduct_Model_Product_Type extends Mage_Catalog_Model_Product_Type_Abstract
{
    const TYPE_KITPRODUCT = 'kitproduct';
    /**
    ...
    public function getRelationInfo()
    {
        ...
        $info->setTable('kitproduct/selection')
    ...
    {
        return Mage::getResourceSingleton('kitproduct/selection')
    ...
    public function getSku($product = null)
    {
        ...
        if ($this->getProduct($product)->getData('sku_type')) {
            ...
            if ($this->getProduct($product)->hasCustomOptions()) {
                $customOption = $this->getProduct($product)->getCustomOption('kitproduct_selection_ids');
    ...
    public function beforeSave($product = null)
    {
        ...
        // If kitproduct product has dynamic weight, than delete weight attribute
        ...
        if ($product->getCanSaveKitProductSelections()) {
            $product->canAffectOptions(true);
            $selections = $product->getKitProductSelectionsData();
            if ($selections) {
                if (!empty($selections)) {
                    $options = $product->getKitProductOptionsData();
                    if ($options) {
     ...
     *
     * @param Mage_Catalog_Model_Product $product
     * @return MagentoAid_KitProduct_Model_Product_Type
     */
    public function save($product = null)
    {
        parent::save($product);
        $product = $this->getProduct($product);
        /* @var $resource MagentoAid_KitProduct_Model_Mysql4_KitProduct */
        $resource = Mage::getResourceModel('kitproduct/kitproduct');
        ...
        if ( $options )
        {
            ...
            try
            {
                ...
                $this->_saveOptions($product, $options);
                $selections = $product->getKitProductSelectionsData();
                if ( $selections )
                {
                    $this->_saveSelections($product, $options, $selections);
                }
    ...
    protected function _saveOptions( $product, &$options )
    {
        //Mage::log( $options );
        ...
        foreach ($options as $key => $option)
        {
            ...
            $optionModel = Mage::getModel('kitproduct/option')
                ->setData($option)
    ...
    protected function _saveSelections( $product, $options, $selections )
    {
        //Mage::log( $selections );
        ...
        foreach ($selections as $index => $group) {
            foreach ($group as $key => $selection) {
                ...
                $selectionProducts = Zend_Json::decode($selectionProducts);
                ...
                $selectionModel = Mage::getModel('kitproduct/selection')
                    ->setData($selection)
                    ->setOptionId($options[$index]['option_id'])
                    ->setParentProductId($product->getId());

                if ( is_array($selectionProducts) )
                if ( count($selectionProducts) )
                {
                    $configurableProductId = (int)$selectionProducts['parent_id'];
                    $selectionModel->setProductId( $configurableProductId );
                }

                $selectionModel->setParentIndex($index);
                $selectionModel->setIndex($key);
        ...
        $resource->saveProductRelations($product->getId(), array_unique($usedProductIds)); // kit <-> configurables
    }

    /**
     * Retrieve kitproduct options items
     *
     * @param Mage_Catalog_Model_Product $product
     * @return array
     */
    ...
    /**
     * Retrieve kitproduct options ids
     *
     * @param Mage_Catalog_Model_Product $product
     * @return array
     */
    ...
    /**
     * Retrieve kitproduct option collection
     *
     * @param Mage_Catalog_Model_Product $product
     * @return MagentoAid_KitProduct_Model_Mysql4_Option_Collection
     */
    public function getOptionsCollection($product = null)
    {
        if (!$this->getProduct($product)->hasData($this->_keyOptionsCollection)) {
            $optionsCollection = Mage::getModel('kitproduct/option')->getResourceCollection()
    ...
    /**
     * Retrive kitproduct selections collection based on used options
     *
     * @param array $optionIds
     * @param Mage_Catalog_Model_Product $product
     * @return MagentoAid_KitProduct_Model_Mysql4_Selection_Collection
     */
    public function getSelectionsCollection($optionIds, $product = null)
    {
        ...
        if (!$this->getProduct($product)->hasData($key)) {
            ...
            $selectionsCollection = Mage::getResourceModel('kitproduct/selection_collection')
                ->addAttributeToSelect(Mage::getSingleton('catalog/config')->getProductAttributes())
                ->addAttributeToSelect('tax_class_id') //used for calculation item taxes in KitProduct with Dynamic Price
    ...
    public function getSelectionsAsAdmin()
    {
        $this->setStoreFilter($this->getProduct()->getStoreId(), $this->getProduct());
        $optionCollection = $this->getOptionsCollection($this->getProduct());
        ...
        $selectionCollection = Mage::getResourceModel('kitproduct/selection_collection')
            ->setOptionIdsFilter($this->getOptionsIds($this->getProduct()));

        $locale->revert();
        Mage::app()->getStore()->setId($store);
        $selectionCollection->getSelect()->order('selection.option_id ASC');
    ...
    /**
     * Checking if we can sale this kitproduct
     *
     * @param Mage_Catalog_Model_Product $product
     * @return bool
     */
     ...
     * Perform standard preparation process and then prepare of kitproduct selections options.
     *
    ...
    protected function _prepareProduct(Varien_Object $buyRequest, $product, $processMode)
    {
        ...
        $_appendAllSelections = true; //(bool)$product->getSkipCheckRequiredOption() || $skipSaleableCheck;
        //var_dump( $_appendAllSelections ); // false


        $options = $buyRequest->getKitProductOption();
        if (is_array($options)) {
            $options = array_filter($options, 'intval');
            $qtys = $buyRequest->getKitProductOptionQty();
            ...
            if (empty($optionIds) && $isStrictProcessMode) {
                return Mage::helper('kitproduct')->__('Please select options for product.');
            }
            ...
            if (!$this->getProduct($product)->getSkipCheckRequiredOption() && $isStrictProcessMode) {
                foreach ($optionsCollection->getItems() as $option) {
                    if ($option->getRequired() && !isset($options[$option->getId()])) {
                        return Mage::helper('kitproduct')->__('Required options are not selected.');
                    }
                }
            }
            $selectionIds = array();
                ...
                foreach ($selections->getItems() as $key => $selection) {
                    if (!$selection->isSalable() && !$skipSaleableCheck) {
                        ...
                        if ($_option->getRequired()
                            ...
                            return Mage::helper('kitproduct')->__('Selected required options are not available.');
                        }
                    }
                }
        ...
        if (count($selections) > 0 || !$isStrictProcessMode) {
            ...
            foreach ($selections as $selection) {
                ...
                $buyRequestConfigurable = $buyRequest['buy_requests'][$selection->getProductId()];

                $simples = Mage::getModel('catalog/product_type_configurable')->getUsedProductCollection( $selection )
                  ->addAttributeToSelect(array('name', 'price'))
                ;
                $simples = $simples->addFieldToFilter('entity_id', array('eq' => (int)$buyRequestConfigurable['product']));
                if ( count($simples) == 0 )
                {
                    return Mage::helper('checkout')->__("Failed to find {$selection->getName()}'s Simple");
                }
                $simple = $simples->getFirstItem();

                $product->addCustomOption('product_qty_' .  $simple->getEntityId(), $qty + $beforeQty,  $simple);


                /*
                 * Create extra attributes that will be converted to product options in order item
                 * for selection (not for all kitproduct)
                 */
                ...
                //$_result = $selection->getTypeInstance(true)->prepareForCart($buyRequestConfigurable, $selection); // $buyRequest $selection
                $_result = $simple->getTypeInstance(true)->prepareForCart($buyRequestConfigurable, $simple);
                ...
                $result[] = $_result[0]->setParentProductId($product->getId())
                    ->addCustomOption('kitproduct_option_ids', serialize(array_map('intval', $optionIds)))
                    ->addCustomOption('kitproduct_selection_attributes', serialize($attributes));
            ...
            // "unique" key for kitproduct selection and add it to selections and kitproduct for selections
            $uniqueKey = implode('_', $uniqueKey);
            $simpleProductIds = array();
            foreach ($result as $item) {
                $item->addCustomOption('kitproduct_identity', $uniqueKey);

                if ( $item->getTypeId() == Mage_Catalog_Model_Product_Type::TYPE_SIMPLE )
                {
                    $simpleProductIds[]= $item->getEntityId();
                }
            }
            $product->addCustomOption('kitproduct_option_ids', serialize(array_map('intval', $optionIds)));
            $product->addCustomOption('kitproduct_selection_ids', serialize($selectionIds));
            $product->addCustomOption('kitproduct_simple_product_ids', serialize($simpleProductIds));
    ...
    {
        return Mage::helper('kitproduct')->__('Please specify Kit product option(s).');
    }

    /**
     * Retrieve kitproduct selections collection based on ids
     *
    ...
    public function getSelectionsByIds($selectionIds, $product = null)
    {
        ...
        if (!$usedSelections || serialize($usedSelectionsIds) != serialize($selectionIds)) {
            $storeId = $this->getProduct($product)->getStoreId();
            $usedSelections = Mage::getResourceModel('kitproduct/selection_collection')
    ...
    /**
     * Retrieve kitproduct options collection based on ids
     *
     * @param array $optionIds
     * @param Mage_Catalog_Model_Product $product
     * @return MagentoAid_KitProduct_Model_Mysql4_Option_Collection
    ...
    public function getOptionsByIds($optionIds, $product = null)
    {
        ...
        if (!$usedOptions || serialize($usedOptionsIds) != serialize($optionIds)) {
            $usedOptions = Mage::getModel('kitproduct/option')->getResourceCollection()
                ->setProductIdFilter($this->getProduct($product)->getId())
                ->setPositionOrder()
    ...
    public function getOrderOptions($product = null)
    {
        ...
        $kitproductOptions = array();

        $product = $this->getProduct($product);

        if ($product->hasCustomOptions()) {
            $customOption = $product->getCustomOption('kitproduct_option_ids');
            $optionIds = unserialize($customOption->getValue());
            $options = $this->getOptionsByIds($optionIds, $product);
            $customOption = $product->getCustomOption('kitproduct_selection_ids');
            $selectionIds = unserialize($customOption->getValue());
            $selections = $this->getSelectionsByIds($selectionIds, $product);
            foreach ($selections->getItems() as $selection) {
                if ($selection->isSalable()) {
                    ...
                    if ($selectionQty) {
                        ...
                        if (!isset($kitproductOptions[$option->getId()])) {
                            $kitproductOptions[$option->getId()] = array(
                        ...
                        $kitproductOptions[$option->getId()]['value'][] = array(
        ...
        $optionArr['kitproduct_options'] = $kitproductOptions;
    ...
    public function getSearchableData($product = null)
    {
        ...
        $optionSearchData = Mage::getSingleton('kitproduct/option')
    ...
    public function checkProductBuyState($product = null)
    {
        ...
        $selectionIds       = $product->getCustomOption('kitproduct_selection_ids');
        $selectionIds       = unserialize($selectionIds->getValue());
        $buyRequest         = $product->getCustomOption('info_buyRequest');
        $buyRequest         = new Varien_Object(unserialize($buyRequest->getValue()));
        $kitproductOption       = $buyRequest->getKitProductOption();

        if (empty($kitproductOption)) {
            Mage::throwException($this->getSpecifyOptionMessage());
        }

        $skipSaleableCheck = Mage::helper('catalog/product')->getSkipSaleableCheck();
        foreach ($selectionIds as $selectionId) {
            /* @var $selection MagentoAid_KitProduct_Model_Selection */
            $selection = $productSelections->getItemById($selectionId);
            if (!$selection || (!$selection->isSalable() && !$skipSaleableCheck)) {
                Mage::throwException(
                    Mage::helper('kitproduct')->__('Selected required options are not available.')
                );
            }
        }
        ...
        $optionsCollection = $this->getOptionsCollection($product);
        foreach ($optionsCollection->getItems() as $option) {
            if ($option->getRequired() && empty($kitproductOption[$option->getId()])) {
                Mage::throwException(
                    Mage::helper('kitproduct')->__('Required options are not selected.')
                );
            }
        }
    ...
    /**
     * Prepare selected options for kitproduct product
     *
     * @param  Mage_Catalog_Model_Product $product
     * @param  Varien_Object $buyRequest
     * @return array
    ...
    {
        $option     = $buyRequest->getKitProductOption();
        $optionQty  = $buyRequest->getKitProductOptionQty();

        $option     = (is_array($option)) ? array_filter($option, 'intval') : array();
        $optionQty  = (is_array($optionQty)) ? array_filter($optionQty, 'intval') : array();

        $options = array(
            'kitproduct_option'     => $option,
            'kitproduct_option_qty' => $optionQty
        );
