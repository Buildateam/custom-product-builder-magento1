...
/**
 * KitProduct products Price indexer resource model
 *
 * @category    Mage
 * @package     MagentoAid_KitProduct
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class MagentoAid_KitProduct_Model_Resource_Indexer_Price extends Mage_Catalog_Model_Resource_Product_Indexer_Price_Default
{
    /**
     * Reindex temporary (price result data) for all products
     *
     * @return MagentoAid_KitProduct_Model_Resource_Indexer_Price
     */
    ...
    {
        $this->_prepareKitProductPrice($entityIds);

        return $this;
    }

    /**
     * Retrieve temporary price index table name for fixed kitproduct products
     *
     * @return string
     */
    protected function _getKitProductPriceTable()
    {
        if ($this->useIdxTable()) {
            return $this->getTable('kitproduct/price_indexer_idx');
        }
        return $this->getTable('kitproduct/price_indexer_tmp');
    }

    /**
     * Retrieve table name for temporary kitproduct selection prices index
     *
     * @return string
     */
    protected function _getKitProductSelectionTable()
    {
        if ($this->useIdxTable()) {
            return $this->getTable('kitproduct/selection_indexer_idx');
        }
        return $this->getTable('kitproduct/selection_indexer_tmp');
    }

    /**
     * Retrieve table name for temporary kitproduct option prices index
     *
     * @return string
     */
    protected function _getKitProductOptionTable()
    {
        if ($this->useIdxTable()) {
            return $this->getTable('kitproduct/option_indexer_idx');
        }
        return $this->getTable('kitproduct/option_indexer_tmp');
    }

    /**
     * Prepare temporary price index table for fixed kitproduct products
     *
     * @return MagentoAid_KitProduct_Model_Resource_Indexer_Price
     */
    protected function _prepareKitProductPriceTable()
    {
        $this->_getWriteAdapter()->delete($this->_getKitProductPriceTable());
        return $this;
    }

    /**
     * Prepare table structure for temporary kitproduct selection prices index
     *
     * @return MagentoAid_KitProduct_Model_Resource_Indexer_Price
     */
    protected function _prepareKitProductSelectionTable()
    {
        $this->_getWriteAdapter()->delete($this->_getKitProductSelectionTable());
        return $this;
    }

    /**
     * Prepare table structure for temporary kitproduct option prices index
     *
     * @return MagentoAid_KitProduct_Model_Resource_Indexer_Price
     */
    protected function _prepareKitProductOptionTable()
    {
        $this->_getWriteAdapter()->delete($this->_getKitProductOptionTable());
        return $this;
    }

    /**
     * Prepare temporary price index data for kitproduct products by price type
     *
     * @param int $priceType
     * @param int|array $entityIds the entity ids limitatation
     * @return MagentoAid_KitProduct_Model_Resource_Indexer_Price
     */
    protected function _prepareKitProductPriceByType($priceType, $entityIds = null)
    {
        $write = $this->_getWriteAdapter();
        $table = $this->_getKitProductPriceTable();
        ...
        if ($priceType == MagentoAid_KitProduct_Model_Product_Price::PRICE_TYPE_DYNAMIC) {
        ...
        if ($priceType == MagentoAid_KitProduct_Model_Product_Price::PRICE_TYPE_FIXED) {
    ...
    /**
     * Calculate fixed kitproduct product selections price
     *
     * @return MagentoAid_KitProduct_Model_Resource_Indexer_Price
     */
    protected function _calculateKitProductOptionPrice()
    {
        $write = $this->_getWriteAdapter();

        $this->_prepareKitProductSelectionTable();
        $this->_calculateKitProductSelectionPrice(MagentoAid_KitProduct_Model_Product_Price::PRICE_TYPE_FIXED);
        $this->_calculateKitProductSelectionPrice(MagentoAid_KitProduct_Model_Product_Price::PRICE_TYPE_DYNAMIC);

        $this->_prepareKitProductOptionTable();

        $select = $write->select()
            ->from(
                array('i' => $this->_getKitProductSelectionTable()),
        ...
        $query = $select->insertFromSelect($this->_getKitProductOptionTable());
        $write->query($query);
        ...
        $select = $write->select()
            ->from(
                array('io' => $this->_getKitProductOptionTable()),
                array('entity_id', 'customer_group_id', 'website_id')
            )
            ->join(
                array('i' => $this->_getKitProductPriceTable()),
    ...
    /**
     * Calculate kitproduct product selections price by product type
     *
     * @param int $priceType
     * @return MagentoAid_KitProduct_Model_Resource_Indexer_Price
     */
    protected function _calculateKitProductSelectionPrice($priceType)
    {
        $write = $this->_getWriteAdapter();
        ...
        $select = $write->select()
            ...
            ->join(
                array('bo' => $this->getTable('kitproduct/option')),
                'bo.parent_id = i.entity_id',
                array('option_id')
            )
            ->join(
                array('bs' => $this->getTable('kitproduct/selection')),
                'bs.option_id = bo.option_id',
                array('selection_id')
            )
            ->joinLeft(
                array('bsp' => $this->getTable('kitproduct/selection_price')),
        ...
        $query = $select->insertFromSelect($this->_getKitProductSelectionTable());
        $write->query($query);
    ...
    /**
     * Prepare temporary index price for kitproduct products
     *
     * @param int|array $entityIds  the entity ids limitation
     * @return MagentoAid_KitProduct_Model_Resource_Indexer_Price
     */
    protected function _prepareKitProductPrice($entityIds = null)
    {
        $this->_prepareTierPriceIndex($entityIds);
        $this->_prepareGroupPriceIndex($entityIds);
        $this->_prepareKitProductPriceTable();
        $this->_prepareKitProductPriceByType(MagentoAid_KitProduct_Model_Product_Price::PRICE_TYPE_FIXED, $entityIds);
        $this->_prepareKitProductPriceByType(MagentoAid_KitProduct_Model_Product_Price::PRICE_TYPE_DYNAMIC, $entityIds);
        ...
        Mage::dispatchEvent('prepare_catalog_product_price_index_table', array(
            'index_table'       => array('i' => $this->_getKitProductPriceTable()),
            'select'            => $select,
        ...
        $this->_calculateKitProductOptionPrice();
    ...
    /**
     * Prepare percentage tier price for kitproduct products
     *
    ...
    protected function _prepareTierPriceIndex($entityIds = null)
    {
        ...
        // remove index by kitproduct products
    ...
    /**
     * Prepare percentage group price for kitproduct products
     *
