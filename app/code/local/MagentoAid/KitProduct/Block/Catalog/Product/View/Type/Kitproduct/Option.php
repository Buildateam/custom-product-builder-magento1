 ...
 */
class MagentoAid_KitProduct_Block_Catalog_Product_View_Type_KitProduct_Option extends MagentoAid_KitProduct_Block_Catalog_Product_Price
{
    /**
    ...
    protected function _getDefaultValues()
    {
        ...
        $inPreConfigured    = $this->getProduct()->hasPreconfiguredValues()
            && $this->getProduct()->getPreconfiguredValues()
                    ->getData('kitproduct_option_qty/' . $_option->getId());
        ...
        if (is_null($this->_selectedOptions)) {
            ...
            if ($this->getProduct()->hasPreconfiguredValues()) {
                $configValue = $this->getProduct()->getPreconfiguredValues()
                    ->getData('kitproduct_option/' . $option->getId());
                if ($configValue) {
    ...
    protected function _getSelectedQty()
    {
        if ($this->getProduct()->hasPreconfiguredValues()) {
            $selectedQty = (float)$this->getProduct()->getPreconfiguredValues()
                ->getData('kitproduct_option_qty/' . $this->getOption()->getId());
            if ($selectedQty < 0) {
                $selectedQty = 0;
            }
        } else {
    ...
    public function setValidationContainer($elementId, $containerId)
    {
        return '<script type="text/javascript">
            ...
            $(\'' . $elementId . '\').callbackFunction  = \'kitproduct.validationCallback\';
            </script>';
    }
    ...
    public function formatPriceString($price, $includeContainer = true)
    {
        ...
        if ($currentProduct->getPriceType() == MagentoAid_KitProduct_Model_Product_Price::PRICE_TYPE_DYNAMIC
