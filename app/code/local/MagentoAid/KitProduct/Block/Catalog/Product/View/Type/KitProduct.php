...
/**
 * Catalog kitproduct product info block
 *
 * @category    Mage
 * @package     MagentoAid_KitProduct
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class MagentoAid_KitProduct_Block_Catalog_Product_View_Type_Kitproduct extends Mage_Catalog_Block_Product_View_Abstract
{
    ...
    public function getOptions()
    {
        if (!$this->_options) {
           ...
           /*$select =  Mage::getSingleton('core/resource')->getConnection('catalog_read')->select()
               ->from(array('selections' => Mage::getResourceModel('kitproduct/selection')->getMainTable()))
               ->where('selection.option_id IN ?',$typeInstance->getOptionsIds($product));
            */
            ...
            $this->_options = $optionCollection->appendSelections($selectionCollection, true,
    ...
    public function getJsonConfig()
    {
        ...
        /* @var $kitproductPriceModel MagentoAid_KitProduct_Model_Product_Price */
        $kitproductPriceModel = Mage::getModel('kitproduct/product_price');
        ...
        foreach ($optionsArray as $_option) {
            /* @var $_option MagentoAid_KitProduct_Model_Option */
            ...
            foreach ($_option->getSelections() as $_selection) {
                ...
                $itemPrice = $kitproductPriceModel->getSelectionFinalTotalPrice($currentProduct, $_selection,
                ...
                if ($currentProduct->getPriceType() == MagentoAid_KitProduct_Model_Product_Price::PRICE_TYPE_FIXED) {
                ...
                Mage::dispatchEvent('kitproduct_product_view_config', $args);
            ...
            if ($preConfiguredFlag) {
                $configValue = $preConfiguredValues->getData('kitproduct_option/' . $optionId);
                if ($configValue) {
        ...
        $config = array(
            'options'       => $options,
            'selected'      => $selected,
            'kitproductId'      => $currentProduct->getId(),
            ...
            'isFixedPrice'  => $this->getProduct()->getPriceType() == MagentoAid_KitProduct_Model_Product_Price::PRICE_TYPE_FIXED,
    ...
    {
        return $this->getProduct()->getTypeInstance()->getSelectionsAsAdmin();
    }
}
