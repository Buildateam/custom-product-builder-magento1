...
/**
 * KitProduct product attributes tab
 *
 * @category    Mage
 * @package     MagentoAid_KitProduct
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class MagentoAid_KitProduct_Block_Adminhtml_Catalog_Product_Edit_Tab_Attributes
    extends Mage_Adminhtml_Block_Catalog_Product_Edit_Tab_Attributes
{
    /**
     * Prepare attributes form of kitproduct product
     *
     * @return void
     */
    protected function _prepareForm()
    {
        ...
        if ($special_price) {
            $special_price->setRenderer(
                $this->getLayout()->createBlock('kitproduct/adminhtml_catalog_product_edit_tab_attributes_special')
        ...
        if ($sku) {
            $sku->setRenderer(
                $this->getLayout()->createBlock('kitproduct/adminhtml_catalog_product_edit_tab_attributes_extend')
                    ->setDisableChild(false)
            );
        }
        ...
        if ($price) {
            $price->setRenderer(
                $this->getLayout()->createBlock('kitproduct/adminhtml_catalog_product_edit_tab_attributes_extend',
                    'adminhtml.catalog.product.kitproduct.edit.tab.attributes.price')->setDisableChild(true)
            );
        }
        ...
        if ($tax) {
            $tax->setAfterElementHtml(
                ...
                function changeTaxClassId() {
                    if ($('price_type').value == '" . MagentoAid_KitProduct_Model_Product_Price::PRICE_TYPE_DYNAMIC . "') {
        ...
        if ($tier_price) {
            $tier_price->setRenderer(
                $this->getLayout()->createBlock('adminhtml/catalog_product_edit_tab_price_tier')
                    ->setPriceColumnHeader(Mage::helper('kitproduct')->__('Percent Discount'))
